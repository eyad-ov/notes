...
class FirebaseDB {
  ...
  final StreamController<List<UserNote>> _noteStreamController = StreamController.broadcast();
  ...
  Stream<QuerySnapshot<Map<String,dynamic>>> _querySnapShotStream (NotesUser user){
    return _firestore
        .collection('notes')
  ...
  Stream<List<UserNote>> userNoteStream (NotesUser user){
    _querySnapShotStream(user).listen((querySnapshot) {
      final  notes = querySnapshot.docs.map((queryDocumentSnapshot){
        ...
        return UserNote(id: id, userId: userId, userEmail: userEmail, text: text);
      }).toList();
